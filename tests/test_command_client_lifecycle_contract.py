...
def test_component_exposes_two_bounded_outbound_command_clients() -> None:
    ...
    assert "NetCommand command_client_command_2_{}" in header
    assert "NetCommandResultObserver *command_result_observers_[2]{}" in header
    assert "USE_ESPNOW_NET_PROTOCOL_DUAL_COMMAND_CLIENT" in header
    assert "bool can_start_command() const" in header
...
def test_progress_is_observed_without_closing_the_transaction() -> None:
    ...
    branch = branch[:branch.index("finish_command_client_(result, slot);")]
...
def test_every_terminal_path_notifies_then_releases_the_generic_client() -> None:
    ...
    assert "clear_command_client_(slot);" in finish
...
def test_timeout_and_delivery_failures_are_terminal_result_events() -> None:
    ...
    assert "finish_command_client_(result, slot)" in failure
...
def test_cancel_releases_sender_and_quarantines_late_result() -> None:
    ...
    assert "command_client_command_2_.transaction_id == transaction_id" in cancel
    assert "sender_.reset();" in cancel
    assert "last_terminal_transaction_id_ = transaction_id" in cancel
    assert "clear_command_client_(slot);" in cancel
